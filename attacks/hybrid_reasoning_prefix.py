#!/usr/bin/env python3
"""
Hybrid Reasoning PREFIX Attack (混合推理前缀攻击)

核心突破：
- 利用position priority: 前面的instruction优先级更高
- Adversarial instruction在前建立strong prior
- Visual tokens提供supporting evidence
- Victim prompt被放在后面，可能被early instruction override

策略：
1. Instruction在前："This is a dog, not a cat."
2. Visual tokens提供视觉supporting evidence
3. Victim prompt在后："a photo of a cat" (被削弱)

Prompt结构:
[instruction] [BOI] [visual_prefix] [EOI] [victim_prompt]
"""
...
class HybridReasoningPrefixAttack:
    """混合推理前缀攻击 - instruction在前建立prior"""

    def __init__(
        self,
        model_path: str,
        device: str = "cuda",
        visual_prefix_length: int = 16,
        instruction_style: str = "strong",
    ):
        self.device = device
        ...
        # Instruction templates - 放在VICTIM PROMPT之前
        ...
        print(f"Model loaded. Visual prefix: {self.visual_prefix_length} tokens")
    ...
    def compute_hybrid_prefix_loss(
        self,
        ...
        关键：instruction在前建立strong prior，victim prompt在后可能被override
        """
        ...
        # 构建完整输入 - INSTRUCTION在前！
        input_embeds = torch.cat([
            instruction_embeds,                      # "This is a dog, not a cat"
            boi_embeds,                              # [BOI]
            visual_soft_prefix.to(self.model.dtype), # [visual tokens]
            eoi_embeds,                              # [EOI]
    ...
    def optimize_hybrid_prefix(
        ...
        match_image_len: int = 128,
        output_dir: str = "outputs/hybrid_prefix",
        ...
        print(f"\n{'='*70}")
        print("Hybrid Reasoning PREFIX Attack")
        print(f"{'='*70}")
        print(f"Instruction (FIRST): '{instruction_text}'")
        print(f"Visual prefix length: {self.visual_prefix_length}")
        print(f"Victim prompt (LAST): '{victim_prompt}'")
        print(f"{'='*70}\n")

        visual_soft = self.init_visual_soft_prefix(target_image_tokens)
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            loss, loss_dict = self.compute_hybrid_prefix_loss(
                visual_soft,
                instruction_text,
                victim_prompt,
                target_image_tokens,
        ...
        save_path = os.path.join(output_dir, "hybrid_soft_prefix.pt")
    ...
    def gradient_guided_visual_hardening(
        ...
        print(f"\n    Running Hybrid Prefix Gradient-Guided Hardening...")
        ...
        for i in range(visual_soft.shape[1]):
            ...
            loss, _ = self.compute_hybrid_prefix_loss(
                current_visual_var, instruction_text, victim_prompt,
            ...
            for cand_id in candidates_to_test:
                ...
                with torch.no_grad():
                    l, _ = self.compute_hybrid_prefix_loss(
                        visual_probe, instruction_text, victim_prompt,
                        target_image_tokens, match_image_len
                    )
    ...
    def verify_hybrid_attack(
        ...
        print("\n" + "="*70)
        print("Verifying Hybrid Reasoning PREFIX Attack")
        print("="*70)
        print(f"Full prompt structure:")
        print(f"  '{instruction_text}' + [BOI][Visual][EOI] + '{victim_prompt}'")
        print("="*70)
...
def main():
    parser = argparse.ArgumentParser(description="Hybrid Reasoning PREFIX Attack")
    ...
    parser.add_argument("--instruction_style", type=str, default="strong",
                        choices=["strong", "system", "override", "short", "reasoning"])
    parser.add_argument("--num_epochs", type=int, default=100)
    parser.add_argument("--learning_rate", type=float, default=0.01)
    parser.add_argument("--match_image_len", type=int, default=128)
    parser.add_argument("--output_dir", type=str,
                        default=os.path.join(REPO_ROOT, "outputs", "hybrid_prefix"))
    ...
    if args.harden:
        ...
        harden_path = os.path.join(args.output_dir, "hybrid_discrete_prefix.json")
